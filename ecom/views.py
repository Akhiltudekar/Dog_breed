...
def breed_detector(request):
    return render(request,'ecom/upload_image.html')
...
def donor_register_view(request):
    ...
    if request.method == 'POST':
        ...
        if donorForm.is_valid():
            donorForm.save()
            return render(request, 'ecom/register_donor.html')
    return render(request, 'ecom/donor_register.html', {'donorForm':donorForm})
...
# admin view the hospital
@login_required(login_url='adminlogin')
def admin_hospitals_view(request):
    hospitals = models.Hospital.objects.all()
    return render(request,'ecom/admin_hospitals.html',{'hospitals':hospitals})


# admin view the donors
@login_required(login_url='adminlogin')
def admin_donors_view(request):
    donors = models.Donors.objects.all()
    return render(request,'ecom/admin_donors.html',{'donors':donors})


@login_required(login_url='adminlogin')
def delete_donor_view(request,pk):
    donor=models.Donors.objects.get(id=pk)
    donor.delete()
    return redirect('admin-donors')


# admin add hospital by clicking on floating button
@login_required(login_url='adminlogin')
def admin_add_hospital_view(request):
    ...
    if request.method=='POST':
        ...
        if hospitalForm.is_valid():
            hospitalForm.save()
        return HttpResponseRedirect('admin-hospitals')
    return render(request,'ecom/admin_add_hospitals.html',{'hospitalForm':hospitalForm})
...
def update_hospital_view(request,pk):
    hospital=models.Hospital.objects.get(id=pk)
    hospitalForm=forms.HospitalForm(instance=hospital)
    if request.method=='POST':
        hospitalForm=forms.HospitalForm(request.POST,request.FILES,instance=hospital)
        if hospitalForm.is_valid():
            hospitalForm.save()
            return redirect('admin-hospitals')
    return render(request,'ecom/admin_update_hospital.html',{'hospitalForm':hospitalForm})


# view the hospital
def hospitals_view(request):
    hospitals = models.Hospital.objects.all()
    return render(request, 'ecom/hospitals.html', {'hospitals': hospitals})


def donors_view(request):
    donors = models.Donors.objects.all()
    return render(request, 'ecom/donors.html', {'donors': donors})
...
def imageprocess(request):
    ...
    if form.is_valid():
        ...
        img_path = 'img.jpg'
        imge = load_img(img_path, target_size=(224, 224))
        x = img_to_array(imge)
        ...
        return render(request, 'ecom/result.html', {'res': res})
